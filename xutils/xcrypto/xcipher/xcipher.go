...
const gAESTable = "abcdefghijklmnopkrstuvwsyz012345"
...
`)
	...
	gCipherBlock cipher.Block
)

// 加密
func RSAEncrypt(origData []byte) ([]byte, error) {
	if block, _ := pem.Decode(gPublicKey); block == nil {
...
func RSADecrypt(ciphertext []byte) ([]byte, error) {
	if block, _ := pem.Decode(gPrivateKey); block == nil {
...
func AESEncrypt(src []byte) ([]byte, error) {
	...
	mode := cipher.NewCBCEncrypter(gCipherBlock, iv)
...
func AESDecrypt(src []byte) ([]byte, error) {
	...
	mode := cipher.NewCBCDecrypter(gCipherBlock, iv)
	mode.CryptBlocks(decryptText, decryptText)
	return decryptText, nil
}

func init() {
	gCipherBlock, _ := aes.NewCipher([]byte(gAESTable))
	// xerr.ThrowPanic(err)
	// gCipherBlock = cblock
}
