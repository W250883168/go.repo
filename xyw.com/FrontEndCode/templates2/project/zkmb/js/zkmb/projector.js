 ...
 */
/*        设备控制 ---投影仪            */
app.controller("projectorController", ['$scope','$timeout','httpService','$location','$modal','$filter', function($scope,$timeout,httpService,$location,$modal,$filter) {
	var timer;
    ...
    $scope.fnGetDeviceStatus = function () {
        ...
        promise.then(function (data) {
            ...
            console.log("获取设备状态",$scope.deviceData)
    ...
    $scope.fnOpenDevice = function () {
        ...
        var url = config.zkmb_config.coapServer + "/device/projector/control/power/on/epson";
		var data = {
			UserID:$scope.uid,        //用户ID：字符串   
    ...
    $scope.fnCloseDevice = function () {
        fnStopAutoRefresh();
        $.blockUI({ message: '<div style="padding: 10px"><span style="font-size: 13px;"> <img src="../project/zkmb/img/zkmb/loading.gif">&nbsp;执行中...</span></div>',timeout:10000 });
        var url = config.zkmb_config.coapServer + "/device/projector/control/power/off/epson";
		var data = {
			UserID:$scope.uid,        //用户ID：字符串   
			DeviceID:$scope.deviceId,    //设备ID：字符串   
			Params:""     //参数：字符串(可选)
