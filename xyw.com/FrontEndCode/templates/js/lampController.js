 ...
 */
app.controller('lampController', ['$scope','httpService','notificService','$location','settings','$window','$interval','$timeout','$filter','$modal','TipService',function ($scope, httpService,notificService,$location,settings,$window,$interval,$timeout,$filter,$modal,TipService) {
    var timer;
    ...
    $scope.fnGetDeviceStatus = function () {
        ...
        promise.then(function (data) {
            ...
            //  状态
			$scope.status($scope.deviceData);     //     2016-10-18 add
    ...
    $scope.fnOpenDevice = function () {
        fnStopAutoRefresh();
        $.blockUI({ message: '<div style="padding: 10px"><span style="font-size: 13px;"> <img src="../img/loading.gif">&nbsp;执行中...</span></div>',timeout:10000 });
