...
func InitDb() *gorp.DbMap {
	db, err := sql.Open("mysql", cfi.Read("data", "datastr")) // "root:root@tcp(localhost:3306)/zndx"
	CheckErr(err, "sql.Open failed")
	dbmap := &gorp.DbMap{Db: db, Dialect: gorp.MySQLDialect{"MyISAM", "UTF8"}}
	CheckErr(err, "Create tables failed")

	if gShowSQL {
		dbmap.TraceOn("[gorp]", log.New(os.Stdout, "", log.LstdFlags|log.Lshortfile))
	}
