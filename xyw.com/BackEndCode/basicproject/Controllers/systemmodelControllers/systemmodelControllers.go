...
import (
	...
	"fmt"
	//	"fmt"
	"basicproject/DataAccess/systemmodelDataAccess"
...
func AddSystemModel(c *gin.Context) {
	...
	xtext.RequireNonBlank(obj.Modulename)
	xtext.RequireNonBlank(obj.Modulecode)
	if errs1 == nil {
		dbmap := core.InitDb()
		defer dbmap.Db.Close()
		lgs, _ := c.Get("users")
...
func ChangeSystemModel(c *gin.Context) {
	...
	xtext.RequireNonBlankMeg(obj.Modulename, "模块名称不能为空")
	xtext.RequireNonBlankMeg(obj.Modulecode, "模块代码不能为空")
	if errs1 == nil {
		dbmap := core.InitDb()
		defer dbmap.Db.Close()
		lgs, _ := c.Get("users")
...
func AddSystemModelFunctions(c *gin.Context) {
	...
	xtext.RequireNonBlankMeg(obj.Functionname, "功能名称不能为空")
	xtext.RequireNonBlankMeg(obj.Functioncode, "功能代码不能为空")
	xnumeric.RequireNonNegativeMeg(obj.Systemmoduleid, "未选定所属模块")
	if errs1 == nil {
		dbmap := core.InitDb()
		defer dbmap.Db.Close()
		lgs, _ := c.Get("users")
...
func AddRoles(c *gin.Context) {
	...
	xtext.RequireNonBlankMeg(obj.Rolesname, "角色名称不能为空")
	if errs1 == nil {
		dbmap := core.InitDb()
		defer dbmap.Db.Close()
		lgs, _ := c.Get("users")
		...
		if rd.Rcode == "1000" {
			isUnique := systemmodelDataAccess.QueryUniqueRolesName(obj.Rolesname, dbmap) //检查唯一约束
			if isUnique > 0 {
				rd.Rcode = "1002"
				rd.Reason = "添加失败:此角色名称已经存在"
			} else {
