...
func QueryUsersPG(ws viewmodel.QueryUsersWhere, pg core.PageData, dbmap *gorp.DbMap) (rd core.Returndata) {
	...
	if sqlerr == nil && countint > 0 {
		...
		sql := "select us.Id as UsersId,Loginuser,Rolesid,IFNULL(us.Loginpwd, '') AS LoginPwd, IFNULL((SELECT roles.Rolesname FROM roles WHERE us.Rolesid = roles.Id), '') AS RoleName, Truename,Nickname,Userheadimg,Userphone,Userstate,Usersex,Usermac,Birthday,"
...
//添加用户
func AddUsersInfoAll(us users.Users, st users.Students, tr users.Teacher, dbmap *gorp.DbMap) (rd core.Returndata) {
	dbmap.AddTableWithName(users.Users{}, "users").SetKeys(true, "Id")
	dbmap.AddTableWithName(users.Students{}, "students")
	dbmap.AddTableWithName(users.Teacher{}, "teacher")
	rd.Rcode = "1001"
	rd.Reason = "数据交互失败"
	countnum, countinerr := dbmap.SelectInt("select count(*) from users where Loginuser=?", us.Loginuser)
	if countnum == 0 && countinerr == nil {
		dbtran, inerr := dbmap.Begin()
		inerr = dbtran.Insert(&us)
		if inerr == nil && us.Rolesid == 3 { //判断是否是学生
			st.Id = us.Id
			inerr = dbtran.Insert(&st)
			core.CheckErr(inerr, "usersDataAccess|AddUsersInfoAll|添加学生详细信息:")
			if inerr != nil {
				dbtran.Rollback()
				return rd
			}
			_, inerr = dbtran.Exec("update classes set Classesnum=Classesnum+1 where Id=?;", st.Classesid) //更新班级人数
			core.CheckErr(inerr, "usersDataAccess|AddUsersInfoAll|更新班级人数:")
			if inerr != nil {
				dbtran.Rollback()
				return rd
			}
			majorid, inerr := dbtran.SelectInt("select Majorid from classes where Id=?", st.Classesid) //获取专业ID
			core.CheckErr(inerr, "usersDataAccess|AddUsersInfoAll|获取专业ID:")
			if inerr != nil {
				dbtran.Rollback()
				return rd
			}
			_, inerr = dbtran.Exec("update major set Majornum=Majornum+1 where Id=?;", majorid) //更新专业人数
			core.CheckErr(inerr, "usersDataAccess|AddUsersInfoAll|更新专业人数:")
			if inerr != nil {
				dbtran.Rollback()
				return rd
			}
			collegeid, inerr := dbtran.SelectInt("select Collegeid from major where Id=?", majorid) //获取学院ID
			core.CheckErr(inerr, "usersDataAccess|AddUsersInfoAll|获取学院ID:")
			if inerr != nil {
				dbtran.Rollback()
				return rd
			}
			_, inerr = dbtran.Exec("update college set Collegenum=Collegenum+1 where Id=?;", collegeid) //更新学院人数
		...
		if countinerr == nil {
			rd.Rcode = "1002"
			rd.Reason = us.Loginuser + ":账户已被添加，账户重复添加不成功"
		}
	}
	return rd
}
...
// 更新用户
func UpdateUsersInfoAll(us *users.Users, st *users.Students, tr *users.Teacher, dbmap *gorp.DbMap) (rd core.Returndata) {
	dbmap.AddTableWithName(users.Users{}, "users").SetKeys(true, "Id")
	...
	rd.Reason = "数据交互失败"
	var old users.Users
	countinerr := dbmap.SelectOne(&old, "select * from users where Loginuser=? and id=?", us.Loginuser, us.Id)
	if old.Id > 0 && countinerr == nil {
		if us.Loginuser == old.Loginuser {
			dbtran, inerr := dbmap.Begin()
			_, inerr = dbtran.Update(us)
			if inerr == nil && us.Rolesid == 3 { //判断是否是学生
				var oldst users.Students
				inerr = dbtran.SelectOne(&oldst, "select * from students where Id=?;", us.Id)
				core.CheckErr(inerr, "usersDataAccess|UpdateUsersInfoAll|查询修改前的信息:")
				if oldst.Classesid != st.Classesid {
					_, inerr = dbtran.Exec("update classes set Classesnum=Classesnum-1 where Id=?;", oldst.Classesid) //更新班级人数
					core.CheckErr(inerr, "usersDataAccess|UpdateUsersInfoAll|更新班级人数:")
					if inerr != nil {
						dbtran.Rollback()
						return rd
					}
					majorid, inerr := dbtran.SelectInt("select Majorid from classes where Id=?", oldst.Classesid) //获取专业ID
		...
		if countinerr == nil {
			rd.Rcode = "1002"
			rd.Reason = us.Loginuser + ":未找到此账号的信息"
		}
	}
