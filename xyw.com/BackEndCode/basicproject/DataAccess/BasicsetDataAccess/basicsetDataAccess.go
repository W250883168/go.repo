...
//查询校区代码是否唯一
func QueryUniqueCampuscode(Campuscode, Campusname string, dbmap *gorp.DbMap) (num int64) {
...
func QueryCollegePG(ws viewmodel.QueryBasicsetWhere, pg core.PageData, dbmap *gorp.DbMap) (rd core.Returndata) {
	...
	if sqlerr == nil && countint > 0 {
		var list []viewmodel.ListCollegeView //[]basicset.College
		sql := "select * from(select cg.Id,cg.Campusid,Collegename,Collegeicon,Collegecode,ifnull(c.Campusname,'')as Campusname from college cg left join Campus c on cg.Campusid=c.id) tb where 1=1 "
		sql = sql + wheresql + " order by Campusid" + core.GetLimitString(pg) + ";"
		_, sqlerr := dbmap.Select(&list, sql)
		core.CheckErr(sqlerr, "basicsetDataAccess|QueryCollegePG|系统后台获取所有学院:")
		rd.Rcode = "1000"
...
func QueryTeachers(ws viewmodel.QueryBasicsetWhere, pg core.PageData, dbmap *gorp.DbMap) (list []viewmodel.Teacher) {
	sql := "select t.Id as Usersid,us.Loginuser,us.Truename,us.Nickname,t.Collegeid,ifnull(t.Majorid,0)as Majorid from teacher as t inner join users as us on t.Id=us.Id "
	sql = sql + " where 1=1 "
	if ws.Collegeid > 0 {
	...
	sql = sql + " group by t.Id,us.Loginuser,us.Truename" + core.GetLimitString(pg) + ";"
	fmt.Println(sql)
	_, Campuserr := dbmap.Select(&list, sql)
