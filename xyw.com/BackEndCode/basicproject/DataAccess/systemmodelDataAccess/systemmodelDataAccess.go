...
//查询角色名称是否唯一
func QueryUniqueRolesName(Rolesname string, dbmap *gorp.DbMap) (num int64) {
	num, _ = dbmap.SelectInt("select count(Rolesname) from Major WHERE (Rolesname=?);", Rolesname)
	return num
}
func InsertRoles(obj systemmodel.Roles, dbmap *gorp.DbMap) (rd core.Returndata) {
	...
	if count > 0 {
		rd.Rcode = "1002"
		rd.Reason = "数据提交错误，此角色名称已经存在不能添加"
	} else {
		inerr := dbmap.Insert(&obj)
